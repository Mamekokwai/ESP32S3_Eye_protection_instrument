...
诊断指令 `GPIO4 <0/1>`、`GPIO5 [0/1]`（别名 `G5`）、`I2CTEST`、`I2CFIX` 只用于产线/示波器排障，不属于正常产品协议。固件没有 `BL` 指令；V1.4 背光由 ESP32 GPIO1（PWM_LED → Q3 → LEDK）控制，默认 100% 亮度，`spilcd_backlight_set()` 可调。
