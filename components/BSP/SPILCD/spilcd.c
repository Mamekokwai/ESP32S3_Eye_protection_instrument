...
{
    /* * 因为 RST 引脚在 spilcd.h 中已设为 -1 (GPIO_NUM_NC), 
     * 所以不再需要手动通过 XL9555 复位，注释掉这些代码
     */
    /*
    LCD_RST(0);
    ...
    */
    ...
    esp_lcd_panel_io_spi_config_t io_config = {
        .dc_gpio_num         = LCD_DC_PIN,          /* DC IO (来自 spilcd.h) */
        .cs_gpio_num         = LCD_CS_PIN,          /* CS IO (来自 spilcd.h) */
    ...
    esp_lcd_panel_dev_config_t panel_config = {
        .reset_gpio_num = LCD_RST_PIN,                  /* RTS IO (来自 spilcd.h, 已设为 -1) */
    ...
    /* 复位LCD (此函数会自动处理 RST_PIN 为 -1 的情况，即不操作复位引脚) */
