...
static esp_err_t panel_jd9855_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data);
static esp_err_t panel_jd9855_invert_color(esp_lcd_panel_t *panel, bool invert_color_data);
static esp_err_t panel_jd9855_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y);
static esp_err_t panel_jd9855_swap_xy(esp_lcd_panel_t *panel, bool swap_axes);
static esp_err_t panel_jd9855_set_gap(esp_lcd_panel_t *panel, int x_gap, int y_gap);
...
esp_err_t esp_lcd_new_panel_jd9855(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    ESP_GOTO_ON_FALSE(jd9855, ESP_ERR_NO_MEM, err, TAG, "no mem for jd9855 panel");
    ...
    jd9855_vendor_config_t *vendor_config = (jd9855_vendor_config_t *)panel_dev_config->vendor_config;
    if (vendor_config) {
...
static esp_err_t tx_param(jd9855_panel_t *jd9855, esp_lcd_panel_io_handle_t io, int lcd_cmd, const void *param, size_t param_size)
{
...
static esp_err_t tx_color(jd9855_panel_t *jd9855, esp_lcd_panel_io_handle_t io, int lcd_cmd, const void *param, size_t param_size)
{
    if (jd9855->flags.use_qspi_interface) {
        lcd_cmd &= 0xff;
        lcd_cmd <<= 8;
...
static esp_err_t panel_jd9855_reset(esp_lcd_panel_t *panel)
{
    ...
    if (jd9855->reset_gpio_num >= 0) {
        ...
        ESP_RETURN_ON_ERROR(tx_param(jd9855, io, LCD_CMD_SWRESET, NULL, 0), TAG, "send command failed");
        vTaskDelay(pdMS_TO_TICKS(120));
    }
...
// WA54TE057I-20Z _JD9855_320x320_MCU8_V1.0_G2.2_20260521
//  {cmd, (uint8_t[]){ data }, data_size, delay_ms}
    {0xDE, (uint8_t[]){0x00}, 1, 20},
    {0xDF, (uint8_t[]){0x98,0x55}, 2, 0},
    {0xB2, (uint8_t[]){0x3C}, 1, 0},
    {0xB7, (uint8_t[]){0x01,0x29,0x01,0x51}, 4, 0},
    {0xBB, (uint8_t[]){0x1B,0x5B,0xD4,0x23,0x3E,0xB6}, 6, 0},
    {0xBC, (uint8_t[]){0x01,0xED,0xF3,0xC0}, 4, 0},
    {0xC0, (uint8_t[]){0x24,0x22}, 2, 0},
    {0xC3, (uint8_t[]){0x00,0x02,0x23,0x0B,0x08,0x47,0x09,0x04,0x62,0x30,0x30}, 11, 0},
    {0xC4, (uint8_t[]){0x40,0x00,0xA8,0x40,0x49,0x05,0x06,0x16,0x49,0x05,0x06}, 11, 0},
    // SET_R_GAMMA_G2.2
    {0xC8, (uint8_t[]){
        0x3F,0x36,0x2E,0x2B,0x2C,0x2E,0x28,0x28,0x26,0x24,0x1F,0x11,0x0C,0x07,0x02,0x00,
        0x3F,0x36,0x2E,0x2B,0x2C,0x2E,0x28,0x28,0x26,0x24,0x1F,0x11,0x0C,0x07,0x02,0x00
    }, 32, 0},
    {0xD3, (uint8_t[]){0x28,0x13}, 2, 0},
    {0xDE, (uint8_t[]){0x01}, 1, 0},
    {0xB7, (uint8_t[]){0x13,0xE7,0x64,0x39,0x06,0x36,0x18,0x18}, 8, 0},
    {0xBE, (uint8_t[]){0x00}, 1, 0},
    {0xC1, (uint8_t[]){0x04,0x4A,0x90,0x08}, 4, 0},
    {0xC2, (uint8_t[]){0x00,0x16,0xDA,0xE7}, 4, 0},
    {0xC7, (uint8_t[]){0x00,0x00,0x08,0x3F,0x08,0x3F}, 6, 0},
    {0xC8, (uint8_t[]){0x00,0x00,0x0B,0x3F,0x08,0x3F}, 6, 0},
    {0xC9, (uint8_t[]){0x02,0x00,0x06,0x04,0x0A}, 5, 0},
    {0xCA, (uint8_t[]){0x08,0x1F,0x1E,0x38,0x37}, 5, 0},
    {0xCB, (uint8_t[]){0x03,0x01,0x07,0x05,0x0B}, 5, 0},
    {0xCC, (uint8_t[]){0x09,0x1F,0x1E,0x38,0x37}, 5, 0},
    {0xCD, (uint8_t[]){0x21,0x23,0x29,0x2B,0x25}, 5, 0},
    {0xCE, (uint8_t[]){0x27,0x3E,0x3F,0x38,0x37}, 5, 0},
    {0xCF, (uint8_t[]){0x20,0x22,0x28,0x2A,0x24}, 5, 0},
    {0xD0, (uint8_t[]){0x26,0x3E,0x3F,0x38,0x37}, 5, 0},
    {0xD1, (uint8_t[]){0x04,0x30,0x23,0x94,0x10}, 5, 0},
    {0xD2, (uint8_t[]){0x0E,0x0E,0x02,0xF0,0xF0}, 5, 0},
    {0xD3, (uint8_t[]){0x3B,0x08,0x08,0x08,0x00,0x00}, 6, 0},
    {0xD4, (uint8_t[]){0x67,0x00,0x00,0x01,0x01,0x01}, 6, 0},
    {0xD5, (uint8_t[]){0x10,0x10,0x07,0x07,0x0F,0x94,0x26}, 7, 0},
    {0xD6, (uint8_t[]){0x00,0x00,0x40}, 3, 0},
    {0xD7, (uint8_t[]){0x03,0x8F,0x20}, 3, 0},
    ...
    {0x29, (uint8_t[]){0x00}, 0, 20},           // Display ON + 20ms
};
...
static esp_err_t panel_jd9855_init(esp_lcd_panel_t *panel)
{
    ...
    ESP_RETURN_ON_ERROR(tx_param(jd9855, io, LCD_CMD_MADCTL, (uint8_t[]) {
        jd9855->madctl_val,
    }, 1), TAG, "send command failed");
    ESP_RETURN_ON_ERROR(tx_param(jd9855, io, LCD_CMD_COLMOD, (uint8_t[]) {
        jd9855->colmod_val,
    }, 1), TAG, "send command failed");
    ...
    if (jd9855->init_cmds) {
        ...
        init_cmds_size = sizeof(vendor_specific_init_default) / sizeof(jd9855_lcd_init_cmd_t);
    }
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ...
        // Send command
        ESP_RETURN_ON_ERROR(tx_param(jd9855, io, init_cmds[i].cmd, init_cmds[i].data, init_cmds[i].data_bytes), TAG, "send command failed");
        vTaskDelay(pdMS_TO_TICKS(init_cmds[i].delay_ms));

        // Check if the current cmd is the "command set" cmd
        if ((init_cmds[i].cmd == JD9855_CMD_SET)) {
            is_user_set = ((uint8_t *)init_cmds[i].data)[0] == JD9855_PARAM_SET ? true : false;
        }
    }
...
static esp_err_t panel_jd9855_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    ESP_RETURN_ON_ERROR(tx_param(jd9855, io, LCD_CMD_CASET, (uint8_t[]) {
        (x_start >> 8) & 0xFF,
        x_start & 0xFF,
        ((x_end - 1) >> 8) & 0xFF,
        (x_end - 1) & 0xFF,
    }, 4), TAG, "send command failed");
    ESP_RETURN_ON_ERROR(tx_param(jd9855, io, LCD_CMD_RASET, (uint8_t[]) {
        (y_start >> 8) & 0xFF,
        y_start & 0xFF,
        ((y_end - 1) >> 8) & 0xFF,
        (y_end - 1) & 0xFF,
...
static esp_err_t panel_jd9855_invert_color(esp_lcd_panel_t *panel, bool invert_color_data)
{
    ...
    ESP_RETURN_ON_ERROR(tx_param(jd9855, io, command, NULL, 0), TAG, "send command failed");
    return ESP_OK;
}
...
static esp_err_t panel_jd9855_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    jd9855_panel_t *jd9855 = __containerof(panel, jd9855_panel_t, base);
    esp_lcd_panel_io_handle_t io = jd9855->io;
    int command = 0;
