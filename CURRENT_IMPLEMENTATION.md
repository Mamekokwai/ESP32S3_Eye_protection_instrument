...
| UART | UART1（RX GPIO44/TX GPIO43）及原生 USB Serial-JTAG 独立双向通信；响应返回来源链路；UART1 默认 GBK、USB 默认 UTF-8，可用 `ENC`/`ENC?` 独立配置和查询；UART1 命令异步转发到 JTAG（`CA51 ` 前缀，不重复执行）；支持背光控制 | `main/app_uart.c` |
...
UART1 收到的完整命令会以 `CA51 ` 前缀异步转发到 USB Serial-JTAG，仅供调试观察，不会在 JTAG 侧重复执行；转发采用有界队列和非阻塞发送，USB 不可用或队列超限时允许丢弃并记录警告。
